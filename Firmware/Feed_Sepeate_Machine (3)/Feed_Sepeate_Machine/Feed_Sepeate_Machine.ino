        ...
        if(UP_BTN_cnt)
        {
            if(SELECT_BTN_mode == SELECT_MODE_MENU)
            {
                ...
                {                   
                    ...
                    else if(set_time_cnt == 1)			con_min[set_hour]++;

					if(con_hour[set_hour] > 23)			con_hour[set_hour] = 23;
					if(con_min[set_hour] > 59)			con_min[set_hour] = 59;
        ...
        if(DOWN_BTN_cnt)
        {
            ...
            if(SELECT_BTN_mode == SELECT_MODE_MENU)
            {
                ...
                else if(BTN_mode == BTN_TIME_MODE)
                {
					...
					if(con_min[set_hour] < 0 || con_min[set_hour] > 59)			    con_min[set_hour] = 0;
        ...
        for(int i=0; i<8; i++)
        {
            if((con_hour[i] == op_hour) && (con_min[i] == op_min))                  // 지정된 시간이 되었을 때
            {
                ...
                if(temp_i == i)                                                     // i == temp_i 일때 
                {
                    ...
                    {
                        open_flag = 1;
                        Serial_speedPacket(0xFE, 0);
                    }
                    else                                                            // 사료가 충분하지 않을때 오픈
                    {
                        if(open_flag == 0)
                        {
                            Serial_speedPacket(0xFE, 0x10F);
                        }
                        else
                        {
                            Serial_speedPacket(0xFE, 0);
...
void Serial_movePacket(unsigned char ID)
{
    unsigned char Check_Sum, Header = 0xFF;
    Serial.write(Header);
    Serial.write(Header);
    ...
    Serial.write(0);
    Serial.write(0);
    Check_Sum = ~(ID + 0x05 + 0x03 + 0x08);
    Serial.write(Check_Sum);
}

void Serial_speedPacket(unsigned char ID, unsigned int Speed)
{
    unsigned char Check_Sum, Header = 0xFF;
    Serial.write(Header);
    Serial.write(Header);
    ...
    Check_Sum = ~(ID + 0x05 + 0x03 + 0x20 + Speed + (Speed>>8));
    Serial.write(Check_Sum);
}
